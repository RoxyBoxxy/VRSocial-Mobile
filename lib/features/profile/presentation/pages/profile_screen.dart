...
ProfileScreen extends StatefulWidget {
  ...
  const ProfileScreen({Key key, this.otherUserId, this.profileUrl, this.coverUrl, this.profileNavigationEnum=ProfileNavigationEnum.FROM_FEED,}) : super(key: key);
...
class _ProfileScreenState extends State<ProfileScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    super.initState();
    _profileCubit.getUserProfile(widget.otherUserId,widget.coverUrl,widget.profileUrl);
  }
  @override
            ...
            return state.when(initial:()=> LoadingBar(),
                ...
                error:(e)=>  Center(
                  child: NoDataFoundScreen(
                    ...
                    icon: AppIcons.personOption(color: AppColors.colorPrimary,size: 40),
                    title: "Profile not found!",message:
                  e.contains("invalid")?"Sorry, we cannot find the page you are looking for.":e,buttonText: "Go Back",),
                ));
          },
        ),
      ),
                  ...
                  return SliverAppBar (
                    ...
                    actions: [
                      ...
                      IconButton(icon: const Icon(Icons.edit), onPressed: () async {
                        ...
                        _profileCubit.changeProfileEntity(snapshot.data.copyWith(backgroundImage: media));
                        await _profileCubit.updateProfileCover(media);

                        },mediaType: MediaTypeEnum.IMAGE,allowCropping: true);
                    ...
                    flexibleSpace: FlexibleSpaceBar (
                      ...
                      background: snapshot.data==null?Container():TopAppBar (
                        otherUserId: snapshot.data.id,
                        otherUser: widget.otherUserId!=null,
                        profileEntity: snapshot.data,profileNavigationEnum: widget.profileNavigationEnum),
                    ),
              ...
              Container (
                  child: RefreshIndicator (
                    ...
                    child: PostPaginationWidget (
                     ...
                     onOptionItemTap:   userPostCubit.onOptionItemSelected, onTapRepost:userPostCubit.repost,),
                  )),
              Container (
                  child: RefreshIndicator(
                    onRefresh: (){
                    ...
                    child: PostPaginationWidget(
                      ...
                      onOptionItemTap: (PostOptionsEnum value, int index) async=> await userMediaCubit.onOptionItemSelected(value, index), onTapRepost:userMediaCubit.repost,),
                  )),
              Container(
                  child: RefreshIndicator(
                    onRefresh: (){
                    ...
                    child: PostPaginationWidget(
                      ...
                      noDataFoundScreen: NoDataFoundScreen(
                        title: "No likes yet!",
                        icon: AppIcons.likeOption(size: 35,color: AppColors.colorPrimary),
                        buttonText: "Go to the homepage",
                        message: "You don’t have any favorite posts yet. All posts that you like will be displayed here.",
                        onTapButton: () {
                          ExtendedNavigator.root.pop();
                          // BlocProvider.of<FeedCubit>(context).changeCurrentPage(ScreenType.home());
                          // ExtendedNavigator.root.push(Routes.createPost);
                      ...
                      onOptionItemTap: userLikesCubit.onOptionItemSelected, onTapRepost:userLikesCubit.repost,),
                  )),
            ],
          ),
        ),
  ...
  num calculateHeight(ProfileEntity data) {
    if(data==null)return 0.toHeight;
    ...
    /// in case of about is more than 40 chars we will increase height
      ...
      if(context.getScreenWidth>320) {
        ...
        if(data.about.length>40)return 450.toHeight;
        else
          // return 500;
