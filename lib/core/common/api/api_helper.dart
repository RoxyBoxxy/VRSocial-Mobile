...
class ApiHelper{
  ...
  ApiHelper(this.localDataSource){
      ...
      onResponse: (res)async{
        ...
        onRequest: (req)async{
          ...
          if(loginResponse!=null){
            ...
            if(req.method=="GET"){
              req.queryParameters.addAll({
                "session_id":loginResponse.authToken
              });
            }
            else {

              var updatedReq=(req.data as FormData)..fields.add(MapEntry("session_id", loginResponse.authToken));
              req.data=updatedReq;
            }
          }
    ...
    dio.interceptors..add(interceptor)..add(LogInterceptor(request: true,responseBody: true,requestBody: true));
      // ..add(PrettyDioLogger(requestBody: true,requestHeader: true,request: true))

  }

  Future<Either<Failure,Response>> post(String path,HashMap<String,dynamic> body,{dynamic headers})async =>
       safeApiHelperRequest(() =>
          dio.post(path,data: FormData.fromMap(body),options: Options(headers: headers)));

  Future<Either<Failure,Response>> get<T>(String path,{Map<String,dynamic> headers,Map<String, dynamic> queryParameters})async =>
       safeApiHelperRequest(() => dio.get(path,options: Options(headers:headers),queryParameters: queryParameters));

  Future<Either<Failure,Response>> put<T>(String path,{Map<String,dynamic> headers,dynamic body})async =>
       safeApiHelperRequest(() => dio.put(path,options: Options(headers:headers),data: body));

  Future<Either<Failure,Response>> safeApiHelperRequest(Future<dynamic> Function() function) async {
    try {
      var response = await function.call();
      if(response.data["code"]==200||response.data["status"]==200)
      return Right(response);
      else if(response.data["code"]==204)
        return left(NoDataFoundFailure(response.data["message"]??"Something went wrong"));
