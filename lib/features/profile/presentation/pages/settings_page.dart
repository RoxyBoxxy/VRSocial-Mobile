...
class _SettingsScreenState extends State<SettingsScreen> {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: BlocListener<UserSettingCubit, CommonUIState>(
          ...
          listenWhen: (c,s)=>s.maybeWhen(orElse: ()=>false,success: (s)=>s is String,error: (e)=>true),
              ...
              return state.when(
                  ...
                  success: (settingEntity) => StreamBuilder<SettingEntity>(
                      ...
                      builder: (context, snapshot) {
                        return snapshot.data==null?const SizedBox():buildHomeUI(snapshot.data);
                      }
                  ),
                  loading: () => LoadingBar(),
  ...
  Widget buildHomeUI(SettingEntity settingEntity) {
    ...
    return [
      header("General"),
      profileItem(
          "Username", "${settingEntity.name} - ${settingEntity.userName}",onTap: (){
      ...
      profileItem("Display Language", allLanguagesMap[settingEntity.displayLanguage],onTap: (){
      ...
      profileItem(
          "Verify my account", "Click to submit a verification request",onTap: (){
        openUpdateBottomSheet(settingEntity,UpdateSettingEnum.VERIFY_MY_ACCOUNT);
      ...
      profileItem("Account Privacy", "Click to set your account privacy ",
        ...
        showModalBottomSheet(
          ...
          shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: const Radius.circular(10))),
            context: context,
            builder: (c) {
              return BlocProvider.value(
                  value: userSettingCubit,
                  child: PrivacyScreen(privacyModels: PrivacyWidgetModel.getPrivacyModels(accountPrivacyEntity: settingEntity.accountPrivacyEntity),));
            });
      }),
      header("Company"),
      ...
      Padding (
        ...
        child: "Log Out".toCaption(fontWeight: FontWeight.w600, fontSize: 15).toCenter(). onTapWidget(() {
  ...
  void openUpdateBottomSheet(SettingEntity settingEntity,UpdateSettingEnum updateSettingEnum) {
    showModalBottomSheet(
        // enableDrag: true,
      isScrollControlled: true,
        ...
        updateSettingEnum: updateSettingEnum,onTapSave: (){
          switch(updateSettingEnum){

            case UpdateSettingEnum.USERNAME:
              if(userSettingCubit.firstNameValidator.text.isEmpty){
                ExtendedNavigator.root.pop();
                userSettingCubit.firstNameValidator.changeData(settingEntity.firstName);
                userSettingCubit.firstNameValidator.textController.text=settingEntity.firstName;
                context.showSnackBar(message: "First name must not be empty",isError: true);
              }
              else if(userSettingCubit.lastNameValidator.text.isEmpty){
                ExtendedNavigator.root.pop();
                userSettingCubit.lastNameValidator.textController.text=settingEntity.lastName;
                userSettingCubit.lastNameValidator.changeData(settingEntity.lastName);
                context.showSnackBar(message: "Last name m"
                    "ust not be empty",isError: true);
              }
              else if(userSettingCubit.userNameValidator.text.isEmpty){
                ExtendedNavigator.root.pop();
                userSettingCubit.userNameValidator.changeData(settingEntity.userName);
                userSettingCubit.userNameValidator.textController.text=settingEntity.userName;
                context.showSnackBar(message: "Username must not be empty",isError: true);
              }
              else {
            ...
            case UpdateSettingEnum.EMAIL:
              if(userSettingCubit.emailValidator.text.isEmpty){
                ExtendedNavigator.root.pop();
                userSettingCubit.emailValidator.changeData(settingEntity.email);
                userSettingCubit.emailValidator.textController.text=settingEntity.email;
                context.showSnackBar(message: "Email must not be empty",isError: true);
              }
              else{
                userSettingCubit.updateUserSettings(updateSettingEnum);
                ExtendedNavigator.root.pop();
            ...
            case UpdateSettingEnum.WEBSITE:
              if(userSettingCubit.websiteValidators.text.isValidUrl){
                userSettingCubit.websiteValidators.changeData(settingEntity.website);
                userSettingCubit.websiteValidators.textController.text=settingEntity.website;
                ExtendedNavigator.root.pop();
                context.showSnackBar(message: "Website address is not valid",isError: true);
              }
              else {
                userSettingCubit.updateUserSettings(updateSettingEnum);
                ExtendedNavigator.root.pop();
            ...
            case UpdateSettingEnum.PASSWORD:
              if(userSettingCubit.oldPasswordValidator.text.isEmpty) {
                context.showSnackBar(message: "Old Password you must not be empty",isError: true);
              }
              else if(userSettingCubit.newPasswordValidator.text.isEmpty){
                context.showSnackBar(message: "New Password you must not be empty",isError: true);
              }
              else if(userSettingCubit.newPasswordValidator.text.isEmpty||userSettingCubit.confirmPasswordValidator.text.isEmpty||userSettingCubit.newPasswordValidator.text!=userSettingCubit.confirmPasswordValidator.text){
                context.showSnackBar(message: "Please make sure password match",isError: true);
              }
              else {
            ...
            case UpdateSettingEnum.DELETE_ACCOUNT:
              ...
              else context.showSnackBar(message: userSettingCubit.deleteAccountValidator.text);
              ExtendedNavigator.root.pop();
              break;
...
class PrivacyWidgetModel {
  ...
  PrivacyWidgetModel._(this.value, this.privacyOptionEnum,{this.isSelected=false});

  static List<PrivacyWidgetModel> getPrivacyModels({AccountPrivacyEntity accountPrivacyEntity}) {
    return [
      PrivacyWidgetModel._("Yes", PrivacyOptionEnum.SEARCH_VISIBILITY,isSelected: accountPrivacyEntity.showProfileInSearchEngine==Strings.privacyYes),
      PrivacyWidgetModel._("No", PrivacyOptionEnum.SEARCH_VISIBILITY,isSelected: accountPrivacyEntity.showProfileInSearchEngine==Strings.privacyNo),

      PrivacyWidgetModel._("EveryOne", PrivacyOptionEnum.CONTACT_PRIVACY,isSelected:
      accountPrivacyEntity.canDMMe==Strings.privacyEveryOne),
      PrivacyWidgetModel._(
          "The People I Follow", PrivacyOptionEnum.CONTACT_PRIVACY,isSelected: accountPrivacyEntity.canDMMe==Strings.privacyPeopleIFollow),

      PrivacyWidgetModel._("EveryOne", PrivacyOptionEnum.PROFILE_VISIBILITY,isSelected: accountPrivacyEntity.canSeeMyPosts==Strings.privacyEveryOne),
      PrivacyWidgetModel._(
          "My Followers", PrivacyOptionEnum.PROFILE_VISIBILITY,isSelected: accountPrivacyEntity.canSeeMyPosts==Strings.privacyMyFollowers),
    ];
  }
  ...
  PrivacyWidgetModel copyWith({String value,PrivacyOptionEnum privacyOptionEnum, bool isSelected}){
    return PrivacyWidgetModel._(value??this.value, privacyOptionEnum??this.privacyOptionEnum,isSelected: isSelected??this.isSelected);
  }
}
