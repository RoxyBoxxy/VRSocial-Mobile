...
class ChatCubit extends Cubit<CommonUIState> {
  ...
  ChatCubit(this.getChatUseCase,
      this.hashTagPagination,
      this.sendChatMessageUseCase, this.deleteMessageUseCase,
      this.deleteAllMessagesUseCase, this.chatPagination) : super(const CommonUIState.success(unit)){
  ...
  sendMessage(String otherUserId)async{
    ...
    var chatRequestModel = MessagesRequestModel(type: "text",message: message.text,userId: otherUserId);
  ...
  sendImage(String path,String otherUserId)async{
    ...
    // animatedKey.currentState.insertItem(0);
    // chatPagination.pagingController..itemList.insert(0, messageText)..notifyListeners();
    var chatRequestModel = MessagesRequestModel(type: "media",message: message.text,userId: otherUserId,mediaUrl: path);
    var either = await sendChatMessageUseCase(chatRequestModel);
    either.fold((l) => emit(CommonUIState.error(l.errorMessage)), (r) {
      //// updating the index value
      // items[0]=messageText.copyWith(time: DateTime.now().getCurrentFormattedTime(),id: r.data.id.toString(),);
  ...
  deleteAllMessages(DeleteChatRequestModel deleteChatRequestModel) async{
    ...
    either.fold((l) => emit(CommonUIState.error(l.errorMessage)), (r) {
      ...
      emit(CommonUIState.success(deleteChatRequestModel.deleteChat?"Chat Deleted Successfully":"Chat Cleared Successfully"));
    });
  }

  // sending last message to the previous screen
  getLastMessage(){
    if(chatPagination.pagingController.itemList==null||chatPagination?.pagingController?.itemList?.isEmpty==true)return '';
    return chatPagination?.pagingController?.itemList[0];
  }
  
  deleteMessage(int index) async{
    emit(const CommonUIState.loading());
    var either = await deleteMessageUseCase(chatPagination.pagingController.itemList[index].messageId);
    either.fold((l) => emit(CommonUIState.error(l.errorMessage)), (r) {

      changeMessageList(chatPagination.pagingController.itemList..removeAt(index));
      emit(CommonUIState.success(r));
    });
  }
