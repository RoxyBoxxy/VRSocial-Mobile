...
class ShowLikeScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final postCubit = useMemoized<PostCubit>(() => getIt<PostCubit>()..showLikesPagination.setPostID(postId));
    return Scaffold(
      appBar: AppBar(
          title: StreamBuilder<int>(
            initialData: 0,
            stream: postCubit.showLikesPagination.itemLength,
            builder: (context, snapshot) => "People who liked (${snapshot.data})"
      ...
      body: PagedListView.separated(
        ...
        builderDelegate: PagedChildBuilderDelegate<PeopleEntity>(
            noItemsFoundIndicatorBuilder: (_) => NoDataFoundScreen(
              ...
              message: 'This post appears to have no likes yet. To like this post, click on like button.',
              icon: AppIcons.likeOption(color: AppColors.colorPrimary,size: 40),
              onTapButton: () {
            ...
            itemBuilder: (BuildContext context, item, int index) => PeopleItem(
                ...
                ).toSlideAnimation(controller)), separatorBuilder: (BuildContext context, int index) =>commonDivider,
      ),
    );
  }
}
