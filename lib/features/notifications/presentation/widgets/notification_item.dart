...
class _NotificationItemState extends State<NotificationItem> with SingleTickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    ...
    return notificationItem(widget.notificationEntity).onTapWidget(() {
      ...
      if(widget.notificationEntity.postId!=null&&widget.notificationEntity.postId!="null"){
        // context.showSnackBar(message: widget.notificationEntity.postId);
        ExtendedNavigator.root.push(Routes.viewPostScreen,arguments: ViewPostScreenArguments(threadID: int.tryParse(widget.notificationEntity.postId),postEntity: null));
      }else{
        ExtendedNavigator.root.push(Routes.profileScreen,arguments: ProfileScreenArguments(otherUserId: widget.notificationEntity.userID));
      }
    },onLongPress: (){
  ...
  Widget notificationItem(NotificationEntity notificationEntity) => AnimatedContainer(
