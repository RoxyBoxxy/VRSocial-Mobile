...
import
'package:colibri/core/common/api/api_constants.dart';
...
class ProfileRepoImpl extends ProfileRepo{
  ...
  @override
  Future<Either<Failure, ProfileEntity>> getProfileData(String userIdOrUsername) async{
    ...
    var either = await apiHelper.get(ApiConstants.profile,queryParameters: HashMap.from(map));
    return either.fold((l) => left(l), (r) {
      var profileResponse = ProfileResponse.fromJson(r.data);
      var profileEntity = ProfileEntity.fromProfileResponse(profileResponse: profileResponse, loginResponse: loginResponse);
        return right(profileEntity);
    });
  }

  @override
  Future<Either<Failure, List<PostEntity>>> getUserPostByCategory(PostCategoryModel model) async{
    var loginResponse = await localDataSource.getUserData();
    var map={
    ...
    var either = await apiHelper.get(ApiConstants.profilePosts,queryParameters: map);
    return either.fold(
            (l) => left(l),
            (r) => right(ProfilePostResponse.fromJson(r.data).data.posts.map((e) => PostEntity.fromProfilePosts(e)).toList()));
  }

  @override
  Future<Either<Failure, List<PostEntity>>> getBookmarks(String offsetId) async{
    ...
    // if(model.offSetId!=null)map.addAll({"offset":model.offSetId.toString()});
    var either = await apiHelper.get(ApiConstants.getBookmarks,queryParameters: map);
    return either.fold(
            (l) => left(l),
            (r) => right(BookmarksResponse.fromJson(r.data).data.map((e) => PostEntity.fromFeed(e)).toList()));
  }

  @override
  Future<Either<Failure, List<FollowerEntity>>> getFollower(PostCategoryModel model) async{
    var loginResponse = await localDataSource.getUserData();
    var map={
      "user_id":model.userId??loginResponse.data.user.userId.toString(),
      "page_size":ApiConstants.pageSize,
    ...
    final either = await apiHelper.get(ApiConstants.fetchFollowers,queryParameters: map);
    return either.fold(
            (l) => left(l),
            (r) => right(FollowersResponse.fromJson(r.data).data.map((e) => FollowerEntity.fromResponse(e,loginResponse.data.user.userId==e.id)).toList()));
  }

  @override
  Future<Either<Failure, List<FollowerEntity>>> getFollowing(PostCategoryModel model) async{
    ...
    var either = await apiHelper.get(ApiConstants.fetchFollowing,queryParameters: map);
    return either.fold(
            (l) => left(l),
            (r) => right(FollowersResponse.fromJson(r.data).data.map((e) => FollowerEntity.fromResponse(e,loginResponse.data.user.userId==e.id)).toList()));
  }
  ...
  @override
  Future<Either<Failure, dynamic>> followUnFollow(String userId) async => await apiHelper.post(ApiConstants.follow,HashMap.from({
      "user_id":userId
    }));
  ...
  Future<Either<Failure, SettingEntity>> getUserSettings() async{
    ...
    return either.fold((l) =>left(l), (settingResponse) => privacyResponse.fold((l) => left(l), (privacy)  => right(SettingEntity.fromSettingResponse(settingResponse,privacy))));
    // var loginResponse = await localDataSource.getUserData();
  }
  ...
  @override
  Future<Either<Failure, dynamic>> updateUserSetting(UpdateSettingsRequestModel model)async {
     var either = await apiHelper.post(ApiConstants.updateUserSettings,HashMap.from(model.toJson()));
     return either.fold((l) => left(l), (r) => right(r));
  }

  @override
  Future<Either<Failure, dynamic>> updatePassword(UpdatePasswordRequest model) async => await apiHelper.post(ApiConstants.changePassword,HashMap.from(model.toJson()));

  @override
  Future<Either<Failure, dynamic>> updatePrivacy(AccountPrivacyEntity model) async => await apiHelper.post(ApiConstants.updatePrivacySettings,HashMap.from(model.toModelJson()));
  ...
  Future<Either<Failure, dynamic>> deleteAccount(String password) async{
     final either = await apiHelper.post(ApiConstants.deleteAccount, HashMap.from({
       "password":password
     }));
  ...
  @override
  Future<Either<Failure, dynamic>> verifyUserAccount(VerifyRequestModel verifyRequestModel) async {
  ...
  Future<Either<Failure, String>> changeLanguage(String lang) async{
     final either = await apiHelper.post(ApiConstants.changeLanguage,HashMap.from({"lang_name":lang}));
     return either.fold(
             (l) => left(l),
  ...
  Future<Either<Failure, String>> updateAvatar(String image) async{
     ...
     final either = await apiHelper.post(ApiConstants.updateAvatar,HashMap.from({
       "avatar":await image.toMultiPart()
     }));
     return either.fold(
             (l) => left(l),
             (r) => right(r.data["message"]));
  ...
  Future<Either<Failure, String>> updateCover(String image) async{
    final either = await apiHelper.post(ApiConstants.updateCover,HashMap.from({
      "cover":await image.toMultiPart()
    }));
    return either.fold(
            (l) => left(l),
            (r) => right(r.data["message"]));
  ...
  Future<Either<Failure, bool>> getLoginMode() async{
    ...
    }else return Left(NoDataFoundFailure(""));
  }
