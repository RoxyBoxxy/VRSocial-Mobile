  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: BlocBuilder<MessageCubit,MessageState>(
        cubit: messageCubit,
        builder: (_,state){
          return state.when(initial: ()=>const LoadingBar(),
                ...
                child: CustomScrollView (
                  slivers: [
                    SliverAppBar (
                      ...
                      pinned: true,
                      centerTitle: true,
                      title: "Messages".toSubTitle1(color: AppColors.textColor,fontWeight: FontWeight.bold),
                      backgroundColor: Colors.white,
                      flexibleSpace: FlexibleSpaceBar(

                        background: ['search messages...'
                            .toSearchBarField(onTextChange: (text){
                              messageCubit.doSearching(text);
                        })
                            .toHorizontalPadding(8)
                            .toContainer(height: 65)].toColumn(mainAxisAlignment: MainAxisAlignment.end),
                      ),
                      // title:
                    ),
                    buildHome()
              ...
              noData:()=>NoDataFoundScreen (
                ...
                title: "No chats yet!",
                message: 'Oops! It looks like you don\'t have any chat history yet. To start chatting with a user, open his profile page, then click on the chat button to start chatting',
              ),
  ...
  Widget buildHome() {
    return StreamBuilder<List<MessageEntity>> (
      ...
      builder: (context, snapshot) {
        return snapshot.data.isEmpty?SliverToBoxAdapter(
          child: StreamBuilder<String> (
            ...
            builder: (context, snapshot) {
              return NoDataFoundScreen (
                ...
                icon: Images.search.toSvg(color: AppColors.colorPrimary,height: 40,width: 40),
                buttonVisibility: false,
                title: "Nothing found!",
                message: 'Could not find anything in your chats history for your search query ${snapshot.data}. Please try again by typing other keywords.',
              );
            }
          ),
        ) : SliverList (
          delegate: SliverChildBuilderDelegate (
                (_, index) => snapshot.data.isEmpty?const SizedBox() : CustomAnimatedWidget (
                  child: OpenContainer<dynamic> (
                    ...
                    confirmDismiss: (direction)async {
                      return  await context.showOkCancelAlertDialog<bool>(
                          ...
                          desc: "Do you want to delete this chat with ${snapshot.data[index].fullName}?"
                              " Please note that this action cannot be undone!",
                          okButtonTitle: "Delete",
                          onTapOk: () async {
                            final result=await messageCubit.deleteMessage(index);
                    ...
                    key: UniqueKey(),
                    child: snapshot.data.isEmpty?const SizedBox():messageItem(entity: snapshot.data[index]),
                  ), openBuilder: (i,c)=>ChatScreen(
                      otherPersonProfileUrl: snapshot.data[index].profileUrl,
                      otherUserFullName: snapshot.data[index].fullName,
                      otherPersonUserId: snapshot.data[index].userId),onClosed: (s)async{
                    // context.showSnackBar(message: s);
                    // if we got cleared value then we will remove the last message only
                    if(s==null)return;
                   if(s is String){
                    if(s=="cleared") messageCubit.clearChat(index);
                   else if(s=='deleted')messageCubit.deleteChat(index);
                   }
                    else {
        ...
        Row(
          children: [
            RichText(
                ...
                text: TextSpan(
                    ...
                    style: context.subTitle1.copyWith(
                        color: Colors.black, fontWeight: FontWeight.w800))).toFlexible(),
            ...
            [
              ...
              5.toSizedBoxHorizontal,
              entity.time
                  .toSubTitle2(fontWeight: FontWeight.w400, fontSize: 10.toSp).toEllipsis.toFlexible()
            ]
                .toRow(
                mainAxisAlignment: MainAxisAlignment.end,
                crossAxisAlignment: CrossAxisAlignment.center)
