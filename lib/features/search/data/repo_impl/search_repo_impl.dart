...
class SearchRepoImpl extends SearchRepo{
  ...
  @override
  Future<Either<Failure,List<HashTagEntity>>> searchHashtag(TextModelWithOffset model) async{
    HashMap<String,String> map=HashMap.from({
      "query":model.queryText,
      "offset":model.offset,
      "page_size":ApiConstants.pageSize.toString()
    });
    var either = await apiHelper.get(ApiConstants.searchHashtags,queryParameters: map);
    return either.fold((l) => left(l), (r) {
      var hashtagResponse = HashtagResponse.fromJson(r.data);
      return right(hashtagResponse.data.map((e) => HashTagEntity.fromHashTag(e)).toList());
    });
  }

  @override
  Future<Either<Failure,List<PeopleEntity>>> searchPeople(TextModelWithOffset model) async{
    ...
    var either = await apiHelper.get(ApiConstants.searchPeople,queryParameters: map);
    return either.fold((l) => left(l), (r) {
      var hashtagResponse = PeopleResponse.fromJson(r.data);
      return right(hashtagResponse.data.map((e) => PeopleEntity.fromPeopleModel(e,loginResponse.data.user.userId==e.id)).toList());
    });
  }

  @override
  Future<Either<Failure,List<PostEntity>>> searchPosts(TextModelWithOffset model) async{
    HashMap<String,String> map=HashMap.from({
      "query":model.queryText,
      "offset":model.offset,
      "page_size":ApiConstants.pageSize.toString()
    });
    var either = await apiHelper.get(ApiConstants.searchPosts,queryParameters: map);
    return either.fold((l) => left(l), (r) {
      var postsResponse = SearcPostsResponse.fromJson(r.data);
      return right(postsResponse.data.map((e) => PostEntity.fromFeed(e)).toList());
    });
  }
