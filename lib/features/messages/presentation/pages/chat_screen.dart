...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({Key key, this.otherPersonUserId, this.otherUserFullName, this.otherPersonProfileUrl}) : super(key: key);
  @override
...
class _ChatScreenState extends State<ChatScreen> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    super.initState();
    chatCubit = getIt<ChatCubit>()..chatPagination.userId=widget.otherPersonUserId;
    chatCubit.chatPagination.searchChat=false;
    PushNotificationHelper.listenNotificationOnChatScreen=(notificationItem){
      chatCubit.changeMessageList(chatCubit.chatPagination.pagingController.itemList..insert(
          0,notificationItem));
          ...
          child: BlocListener<ChatCubit,CommonUIState> (
            ...
            listener: (BuildContext context, state) {
              state.maybeWhen(orElse: (){},success: (s) {
                if(s is String) {
                  if(s.toLowerCase().contains("cleared"))chatCleared=true;
              ...
              },error: (e)=>context.showSnackBar(isError: true,message: e));
            },
            child: BlocBuilder<ChatCubit,CommonUIState> (
              cubit: chatCubit,
              builder: (c,state)=>state.maybeWhen(orElse: ()=>const LoadingBar(),
        ...
        Positioned(
          ...
          children: [
            TextField(
              ...
              suffixIcon: SvgPicture.asset(Images.chatImage,height: 10,width: 10,).toPadding(14).onTapWidget(() async{
            ...
            //         borderRadius: BorderRadius.circular(40))
            // ).toFlexible(),
            SvgPicture.asset(Images.sendIcon,height: 24,).toHorizontalPadding(12)
                .onTapWidget(() async{
               ...
               if(chatCubit.message.text.trim().isNotEmpty){
                 ...
                 context.showSnackBar(message: "Please enter a valid text",isError: true);
               }
            },removeFocus: false)
            // ,
          ],
    ...
    return RefreshIndicator(
              ...
              child: PagedListView(
                ...
                builderDelegate: PagedChildBuilderDelegate<ChatEntity>(
                  noItemsFoundIndicatorBuilder: (i)=>NoDataFoundScreen(onTapButton: (){
                  ...
                  itemBuilder: (BuildContext context, item, int index) =>Container (
                    ...
                    ReceiverChatItem (otherUserProfileUrl: widget.otherPersonProfileUrl, chatEntity:item)
                  ).toSwipeToDelete(key: UniqueKey(),onDismissed: (){
  ...
  AnimatedList buildAnimatedList(BuildContext context, List<ChatEntity> chat, AsyncSnapshot<List<ChatEntity>> snapshot) {
    ...
    return AnimatedList(
                          ...
                          itemBuilder: (c,index,animation)=>SizeTransition(
                            ...
                            width: context.getScreenWidth,
                            child: chat[index].isSender?SenderChatItem(
                                chatEntity:chat[index]):ReceiverChatItem(
                              otherUserProfileUrl: widget.otherPersonProfileUrl,
                                chatEntity:chat[index]),).toSwipeToDelete(key: ValueKey(index),onDismissed: (){
                                  chatCubit.deleteMessage(index);
                            // context.showSnackBar(;
  ...
  Widget buildFloatingSearchBar() {
    ...
    return FloatingSearchBar(
      ...
      hint: 'search messages...',hintStyle: context.subTitle2.copyWith(fontWeight: FontWeight.w600),
      ...
      leadingActions: [BackButton(onPressed: (){

        // if(isPortrait)
        if(floatingSearchBarController.isClosed)
        navigateToBackWithResult();
        else floatingSearchBarController.close();
        // else
      ...
      actions: [
        ["Delete Chat", "Clear Chat"].toPopUpMenuButton(onTapOptions,icon: RotatedBox(
            quarterTurns: 45,
            child: AppIcons.optionIcon)),
  ...
  onTapOptions(String value)async{
    if(value=="Delete Chat"){
      return  await context.showOkCancelAlertDialog<bool>(
          ...
          desc: "Do you want to delete this chat with ${widget.otherUserFullName}? Please note that this action cannot be undone!",
          okButtonTitle: "Delete",
          onTapOk: ()async{
            // var result=await messageCubit.deleteMessage(index);
            Navigator.of(context).pop();
            chatCubit.deleteAllMessages(DeleteChatRequestModel(userId: widget.otherPersonUserId,deleteChat: true));
          }
      );
      ...
      return  await context.showOkCancelAlertDialog<bool>(
          title: "Please confirm your actions!",
          desc: "Are you sure you want to delete all messages in the chat with ${widget.otherUserFullName}?"
              " Please note that this action cannot be undone!",
          okButtonTitle: "Clear",
          onTapOk: ()async{
            // var result=await messageCubit.deleteMessage(index);
            Navigator.of(context).pop();
            chatCubit.deleteAllMessages(DeleteChatRequestModel(userId: widget.otherPersonUserId,deleteChat: false));
          }
      );
    }
  }
  ...
  void navigateToBackWithResult() {
    ...
    else ExtendedNavigator.root.pop(chatCubit.getLastMessage());
  }
