...
class _PromotedWidgetState extends State<PromotedWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(child: [
      "Sponsored".toHeadLine6(fontWeight: FontWeight.bold).toHorizontalPadding(4),
      [
        item.adTitle.toCaption(fontWeight: FontWeight.bold),
        5.toSizedBox,
        item.adSubTitle.toCaption(fontWeight: FontWeight.bold),
        5.toSizedBox,
        item.adMediaUrl.toNetWorkOrLocalImage(borderRadius: 0,width: context.getScreenWidth,height: 150),
        10.toSizedBox,
        [
          [item.bodyText.toCaption(),
            item.adWebsite.toCaption(fontWeight: FontWeight.bold,color: AppColors.colorPrimary)].toColumn().toFlexible(),
    ...
    ].toColumn(),).toPadding(8).toHorizontalPadding(8);
  }
