...
class AuthRepoImpl extends AuthRepo{
  ...
  @override
  Future<Either<Failure, dynamic>> signIn(HashMap<String, dynamic> hashMap) async{
    var response= await apiHelper.post(ApiConstants.loginEndPoint,hashMap);
    return response.fold((l) {
      if(l.errorMessage.toLowerCase().contains("incorrect")){
        return left(ServerFailure("The email and password you entered does not match. Please double-check and try again"));
      }
      return left(l);
    }, (r) async{
      // saving data locally
  ...
  Future<Either<Failure, String>> fbLogin() async{
    ...
    if(result.status==FacebookLoginStatus.loggedIn) {
      var r= await apiHelper.post(ApiConstants.oauth,HashMap.from({
    ...
    else if(result.status==FacebookLoginStatus.cancelledByUser) return Left(ServerFailure(""));
