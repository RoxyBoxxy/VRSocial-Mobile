...
class PostPaginationWidget extends StatefulWidget {
  ...
  const PostPaginationWidget({Key key,
   @required  this.isComeHome,
   @required  this.pagingController,
   @required  this.onTapLike,
   @required  this.onTapRepost,
   @required  this.onOptionItemTap, this.noDataFoundScreen, this.isSliverList=true, this.isFromProfileSearch = false,
...
class _PostPaginationWidgetState extends State<PostPaginationWidget> {
  ...
  @override
  Widget build(BuildContext context) => widget.isSliverList?PagedSliverList.separated (
      pagingController: widget.pagingController,
      builderDelegate: buildHome(),
      separatorBuilder: (BuildContext context, int index) => Container(),
    ):PagedListView(pagingController: widget.pagingController, builderDelegate: buildHome());

  PagedChildBuilderDelegate<PostEntity> buildHome()=> PagedChildBuilderDelegate<PostEntity>(
        firstPageErrorIndicatorBuilder: (c) {
          // print("hello ${widget?.isPrivateAccount}" );
                      widget?.isPrivateAccount(widget.pagingController.error=='This profile data is not available for viewing');
          return widget.pagingController.error=='This profile data is not available for viewing'?PrivacyProtectedErrorView():CustomFirstPageView(onTryAgain: (){
            widget?.pagingController?.refresh();
          });
        },
        noItemsFoundIndicatorBuilder: (_) => widget.noDataFoundScreen?? NoDataFoundScreen (
          onTapButton: () {
          ...
          return OpenContainer<PostEntity> (
            ...
            closedBuilder: (c, opencontainer) => PostItem   (
              ...
              replyCountIncreased: (value){
                ...
                widget.pagingController..itemList[index]=currentItem.copyWith(commentCount: currentItem.commentCount.inc.toString())
                ..notifyListeners();
              },
...
class PrivacyProtectedErrorView extends StatelessWidget {
  @override
  Widget build(BuildContext context) => Container(child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        const Icon(Icons.privacy_tip_outlined,color: AppColors.colorPrimary,size: 40,),
        10.toSizedBox,
        'This profile is protected'.toSubTitle1(fontWeight: FontWeight.w600),
        10.toSizedBox,
        'Only approved followers can see the posts and content of this profile, click the (Follow) button to see their posts!'.toSubTitle2(align: TextAlign.center),
        10.toSizedBox,
        "Go Back".toButton(color: AppColors.colorPrimary).toHorizontalPadding(24).toOutlinedBorder(() {
          ExtendedNavigator.root.pop();
        })
      ],
